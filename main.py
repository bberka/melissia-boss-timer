...
def convert(boss,value):
  ...
  elif rounded < 60 and rounded > 0 or rounded/60 == 0: return boss.upper() + " in " + str(rounded) + " sec"
  elif rounded < 3600 and rounded > 60: return boss.upper() + " in " + str(int(rounded/60)) + " mins"
  elif rounded > 3600: return boss.upper() + " in " + str(int(rounded/3600)) + " hours and " + str(int((rounded%3600)/60)) + " mins"
...
@tasks.loop(minutes = 1)
async def test_loop():
  _channel = bot.get_channel(849960505649135626)   
  ...
  if len(temp) != 0:
    ...
    for x in temp:
      ...
      if temp == temp0 and db["msg_ntf"] != 0:
        z = discord.Embed(title= boss.upper() + " spawned!",description="",color=color_list[boss.lower()])
        ch = db["msg_ntf"]
        await ch.edit(content=tag,embed=z) 
        db["msg_ntf"] = 0       
      else:
        db["msg_ntf"] = await _channel.send(tag,embed=embedVar)
        if temp == temp15 or temp == temp30:
          db["msg_ntf"] = 0
      print("notif sent test")
...
async def on_message(ctx):
  ...
  if ctx.author == client.user :
    ...
    if msg == prefix + 'boss':      
      ...
      else: await ctx.channel.send(ctx.author.mention + ' please enter a valid boss name.', delete_after=10.0)

    elif msg == prefix + 'test':
      info_channel = bot.get_channel(849960505649135626)   
      ctime = datetime.now(UTC).strftime("%Y-%m-%d %H:%M:%S")
      x = discord.Embed(title= "BOSS TIMES LIST | " + str(ctime) + " UTC +0",description = get_view(get_boss_list()),color=0xffffff)
      await info_channel.send(embed = x) 
      ...
      x.add_field(name="!calendar *TIMEZONE*", value="I will DM you with all boss spawn times. Enter timezone as number value.", inline=False)
      x.add_field(name="!boss", value="I will DM you with bosses remaining time to spawn also you can find that information in here <#850077285356535839>", inline=False)
      x.add_field(name="!boss *ANYBOSSNAME*", value="I will DM you with the boss list that you wrote and their remaining time to spawn.", inline=False)
      x.set_footer(text= "This bot is running on development server so bot reactions could be slow")
